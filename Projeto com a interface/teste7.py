#Corrigir todos os problemas que existiam, como sobronome obrigatorio, variavel de entrada 'Estado' e possibilidade de clicar no link de perfil
...
def formatar_linguagem(linguagem):
    if linguagem.lower() == 'c#':
        ...
        # Se não for C# ou C++, retorna a própria linguagem
...
def consultar():
    global ultimos_perfis, pagina_atual
...
def mostrar_projetos(perfil_url):
    ...
    # Define a URL do avatar do usuário
    avatar_url = f'https://github.com/{nome_usuario}.png'
    ...
    perfil_label = customtkinter.CTkLabel(master=perfil_frame, text=perfil_url, text_color="blue", fg_color="transparent", cursor="hand2")
    perfil_label.pack(side="left", padx=5, pady=5)
    perfil_label.bind("<Button-1>", lambda e, url=perfil_url: abrir_link(url))

    botao_projetos = customtkinter.CTkButton(master=perfil_frame, text="Projetos", text_color="white", fg_color="#0D1E40", font=("Arial", 12), hover_color="#144673", border_width=2, corner_radius=5, command=lambda: exibir_projetos(nome_usuario))
...
def atualizar_pagina(numero_pagina):
    global pagina_atual
...
estado = customtkinter.CTkEntry(master=grid_entradas, placeholder_text="Estado", placeholder_text_color="#0D1E40", fg_color="white", text_color="black", border_color="#0D1E40", border_width=2, width=400, corner_radius=10)
