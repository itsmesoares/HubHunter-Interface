#Consegui integrar a interface com o codigo, mas não esta visualmente bonito
...
def buscar_projetos_por_linguagem(nome_usuario, linguagem):
...
def consultar():
    ...
    for perfil in perfis:
        ...
        perfil_label.pack(anchor="w", padx=5, pady=5)
        perfil_label.bind("<Button-1>", lambda e, url=perfil: abrir_link(url))

        nome_usuario = perfil.split('/')[-1]
        projetos = buscar_projetos_por_linguagem(nome_usuario, linguagem_prog)
        if projetos:
            for projeto in projetos:
                projeto_label = customtkinter.CTkLabel(master=perfil_frame, text=projeto, text_color="blue", fg_color="transparent", cursor="hand2")
                projeto_label.pack(anchor="w", padx=20, pady=2)
                projeto_label.bind("<Button-1>", lambda e, url=projeto: abrir_link(url))
        else:
            projeto_label = customtkinter.CTkLabel(master=perfil_frame, text=f"{nome_usuario} não possui projetos em {linguagem_prog}.", text_color="black", fg_color="transparent", padx=20)
            projeto_label.pack(anchor="w", padx=20, pady=2)
...
# Adicionando um canvas e scrollbar para resultados dentro de um frame
